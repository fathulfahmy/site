      ...
      <Link key={post.slug} href={`/projects/${post.slug}`}>
        <div className="flex justify-between">
          ...
          <div className="mb-1 flex gap-1">
            <p className="bg-neutral-900 p-0.5 text-white xl:text-lg">{new Date(post.date).getFullYear()}</p>
            <p className="bg-neutral-900 p-0.5 text-white xl:text-lg">{post.category}</p>
          </div>
        </div>
